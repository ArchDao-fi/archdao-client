# Treasury

## Example Usage

```typescript
import { Treasury } from "@archdao/archdao-client/models/components/treasury.js";

let value: Treasury = {};
```

## Fields

| Field                                                                  | Type                                                                   | Required                                                               | Description                                                            |
| ---------------------------------------------------------------------- | ---------------------------------------------------------------------- | ---------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| `id`                                                                   | *number*                                                               | :heavy_minus_sign:                                                     | N/A                                                                    |
| `monthlyBudget`                                                        | *number*                                                               | :heavy_minus_sign:                                                     | N/A                                                                    |
| `address`                                                              | *string*                                                               | :heavy_minus_sign:                                                     | N/A                                                                    |
| `tokenSupplyLp`                                                        | *number*                                                               | :heavy_minus_sign:                                                     | N/A                                                                    |
| `usdcSupplyLp`                                                         | *number*                                                               | :heavy_minus_sign:                                                     | N/A                                                                    |
| `tokenReserve`                                                         | *number*                                                               | :heavy_minus_sign:                                                     | N/A                                                                    |
| `usdcReserve`                                                          | *number*                                                               | :heavy_minus_sign:                                                     | N/A                                                                    |
| `status`                                                               | [components.TreasuryStatus](../../models/components/treasurystatus.md) | :heavy_minus_sign:                                                     | N/A                                                                    |