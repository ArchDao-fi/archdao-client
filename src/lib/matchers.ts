...
import { ArchError } from "../models/errors/archerror.js";
...
): MatchFunc<T, E | ArchError | ResponseValidationError> {
  return async function matchFunc(
    response: Response,
    request: Request,
    options?: { resultKey?: string; extraFields?: Record<string, unknown> },
  ): Promise<
    [result: Result<T, E | ArchError | ResponseValidationError>, raw: unknown]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new ArchError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new ArchError("API error occurred", { request, response, body }),
      }, raw];
    }
